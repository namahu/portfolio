...
export function ProjectCard({ type = "personal", project }: ProjectCardProps) {
  ...
  return (
    <div
      className="w-full h-full bg-zinc-200 px-2 py-4 border border-zinc-300 rounded-xl flex flex-col gap-2"
    >
      {/* Card Title */}
      <h3>{type === "oss" ? project.repository?.name : project.title}</h3>

      {/* Card Body */}
      <div className="mx-4 flex-1 flex flex-col justify-between gap-2">
        <div className="flex flex-col gap-2">
          ...
          {type === "oss" ? (
            <div className="flex flex-col gap-2 w-full">
              ...
              <ul className="w-full max-h-24 overflow-y-auto flex flex-col gap-1 pr-1">
                {project.repository?.issues?.map((issue) => (
                  <li key={issue.number} className="w-full">
                    <a
                      ...
                      className="block w-full bg-zinc-50 border border-zinc-300 rounded-md px-2 py-1 truncate hover:bg-zinc-100"
                    >
                      {issue.title}
                    </a>
                  </li>
